...
EC_FILEMASK = '/Data/sim/data/ECMWF_forecast_arctic/ec2_start%s.nc'
EC_TIME_RECS_PER_DAY = 4
...
def deaccumulate_ecmwf(var):
    ...
    Some values in old ECMWF files from MET.NO are provided as accumulated over
    the previous 6h.
    This function calculates cumulative for the last hour.

    Parameters
    ----------
    var : numpy.ndarray
        ECMWF var accumulated over previous 6h
        this has already been interpolated onto high frequency time grid,
        but we just need to use numpy.gradient to get the difference

    Returns
    ------
    var : numpy.ndarray
        ECMWF var accumulated for one hour
    """
    #var_deacc = np.array(var)
    #var_deacc[1:] = np.diff(var, axis=0) # keep the 1st value as is
    #return var_deacc

    # just convert from eg 6h integration to 3h
    return (EC_TIME_RECS_PER_DAY/AR_TIME_RECS_PER_DAY)*var

def accumulate_ecmwf(var):
    """ Calculate accumulated ECMWF variable
    Some values in old ECMWF files from MET.NO are provided as accumulated over
    the previous 6h.
    This function calculates relative to -1*T_arome
    ...
    var : numpy.ndarray
        ECMWF var accumulated for one hour
    """
    var_accum = np.array(var)
    var_accum[0] *= EC_TIME_RECS_PER_DAY/AR_TIME_RECS_PER_DAY
    return np.cumsum(var_accum, axis=0)
...
def precipitation_amount_acc(ec_TP):
    ...
    precipit : numpy.ndarray
        Accumulated precipitation_amount in 1 hour [kg/m^2]
    """
    rho = 1000. # water density [kg / m3]
    ec_TP = deaccumulate_ecmwf(ec_TP)
    precipit = ec_TP * rho
...
def integral_of_surface_downwelling_longwave_flux_in_air_wrt_time(ec_2T, ec_TCC):
    """ Calculate specific humidity for the atmosphere
    ...
    ec_2T : numpy.ndarray
        air_temperature [K]
    ec_TCC : numpy.ndarray
        cloud_area_fraction [1]

    Returns
    ------
    Qlw_in : numpy.ndarray
        Accumulated surface LW downwelling radiation in 1 hour [J/m^2]

    Note
    ----
    # from https://github.com/nansencenter/nextsim/blob/b700af5c67ead220eb42644207ce71109cd731b2/model/finiteelement.cpp#L5456
    ...
    Qlw_in = (sigma_sb * ec_2T**4
            * (1. - 0.261 * np.exp(-7.77e-4 * (ec_2T - _KELVIN)**2 ))
            * (1. + 0.275 * ec_TCC ))
...
def open_ecmwf(date):
    ...
    ec_time_vec = np.linspace(0, 24, EC_TIME_RECS_PER_DAY+1)
...
def set_destination_coordinates(ar_proj, ar_ds):
    ...
    dst_vec = {
        ...
        'time': np.linspace(0, 24, AR_TIME_RECS_PER_DAY+1)[:-1],
...
def export(outfile, ar_ds, dst_ecp, dst_vec, dst_shape):
    ...
    dst_ds.variables['time'][:] = ar_ds.variables['time'][:AR_TIME_RECS_PER_DAY]
...
if __name__ == '__main__':
    ...
    for dst_var_name in DST_VARS:
        ...
        for i_ens in range(num_ens_mems):
            ...
            #ar_var[:] = ar_ds[dst_var_name][:AR_TIME_RECS_PER_DAY, 0, i_ens, :, :]
            ar_var[:] = ar_ds[dst_var_name][:AR_TIME_RECS_PER_DAY, i_ens, :, :]
        ...
        for ec_var_name in ec_var_names:
            ...
            #ec_var = EC_PREPROC[ec_var_name](
            #        ec_ds.variables[ec_var_name][:EC_TIME_RECS_PER_DAY+1,:,:])
            ec_var = ec_ds.variables[
                    ec_var_name][:EC_TIME_RECS_PER_DAY+1,:,:] #no more preproc needed
            dst_ecd_grd = interpolate(ec_var, ec_pts, dst_ecp, dst_shape)
            EC_DATA[ec_var_name] = dst_ecd_grd
        if 0:
            #for testing
            print('Saving %s.npz' %dst_var_name)
            da = deaccumulate_ecmwf(dst_ecd_grd)
            np.savez('%s.npz' %dst_var_name,
                    ec_STRD_acc=dst_ecd_grd,
                    ec_STRD_deacc=da,
                    ar_STRD=dst_ard_grd_all_members[0],
                    )
